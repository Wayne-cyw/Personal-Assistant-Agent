...
def _render_data(response_type: str, data: dict[str, object] | None) -> str | None:
    """Render the `data` payload for booking-related response types. Returns
    None when there's nothing extra to show (plain messages/refusals).
    """
    if not data:
        return None
    ...
    if response_type == "booking_confirmation_request":
        slot = data.get("slot", {})
        return (
            f"Please confirm: {slot} ({data.get('timezone')})\n"
            f"  Name: {data.get('name')}  Email: {data.get('email')}"
        )

    if response_type == "booking_confirmed":
        next_steps = data.get("next_steps", "")
        return f"Booking confirmed (id: {data.get('booking_id')}). {next_steps}".strip()
...
def send_turn(client: httpx.Client, session_id: str, message: str, timezone: str | None) -> str:
    """POST one turn to /v1/chat and return the text to print. Raises
    httpx.ConnectError if the server is unreachable — the caller decides
    how to display that.
    """
