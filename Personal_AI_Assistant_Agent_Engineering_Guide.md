...
Retention: a scheduled cleanup script deletes `messages` and `sessions` older than the configured retention period (default 90 days); `bookings` persist until the owner deletes them.
...
- [ ] Settings added to `config.py`: `WINDOW_HIGH_TOKENS` (3000), `WINDOW_LOW_TOKENS` (1500), `SUMMARY_MAX_TOKENS` (150), `SUMMARIZER_MODEL`, `PINNED_FACTS_MAX` (5).
